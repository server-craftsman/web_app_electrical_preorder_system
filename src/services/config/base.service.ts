...
export const BaseService = {
  ...
  uploadFile: async (file: File, type: "video" | "image", isLoading: boolean = true) => {
    ...
    try {
      ...
      helper.notificationMessage(error instanceof Error ? error.message : "Upload failed", "error");
      return null;
    ...
    if (response) {
      switch (response.status) {
        ...
        case HTTP_STATUS.FORBIDDEN:
          helper.notificationMessage("Access denied. You do not have permission to perform this action.", "error");
          storage.clearLocalStorage();
          setTimeout(() => {
            window.location.href = ROUTER_URL.LOGIN;
          }, 3000);
        ...
        case HTTP_STATUS.INTERNAL_SERVER_ERROR:
          helper.notificationMessage("Internal server error. Please try again later.", "error");
          break;
        default:

          helper.notificationMessage(response.data?.message || "An error occurred. Please try again.", "error");

      }
    } else {
      helper.notificationMessage(err.message || "An error occurred. Please try again.", "error");
    }
